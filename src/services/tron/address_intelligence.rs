...
pub fn build_address_profiles(
    transfers:&[SimpleTransfer],
)->HashMap<String,AddressProfile>
{
    let mut profiles=
        HashMap::<String,AddressProfile>::new();
    ...
    in profiles.iter_mut()
    {
        ...
        if profile.unique_senders >= 100
            && profile.total_in_tx >= 500
        {
            profile.probable_sweeper = true;
            profile.risk_score += 0.3;
        }

        if profile.unique_receivers >= 100
            && profile.total_out_tx >= 500
        {
            profile.probable_exchange = true;
            profile.risk_score += 0.5;
        }

        if profile.total_in_tx >= 10
            && profile.total_out_tx <= 3
        {
