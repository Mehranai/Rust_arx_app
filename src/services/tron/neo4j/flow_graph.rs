...
use super::edges::merge_transfer_edge;
...
pub async fn build_wallet_flow_graph(
    ...
    let edges =
        load_relationship_neighborhood(
            clickhouse.clone(),
            address,
            depth,
            per_address_limit,
    ...
    for node_id in &node_ids {
        if let Some(exchange) =
            load_exchange_metadata(&clickhouse, node_id).await?
        {
    ...
    let incoming_origins =
        incoming_origin_nodes(address, &nodes, &edges);

    let exchange_interactions =
        exchange_summaries(address, &edges, &metadata);
...
async fn load_relationship_neighborhood(
    ...
    let mut queue =
        VecDeque::<(String, u8)>::from([(address.to_string(), 0)]);
    ...
    while let Some((current, current_depth)) = queue.pop_front() {
        ...
        let rows = load_relationships_for_address(
            &clickhouse,
            &current,
            per_address_limit,
        )
    ...
    for edge in edges {
        if edge.from == address {
            if let Some(exchange) = metadata.get(&edge.to) {
                summaries.push(summary_from_edge(
                    edge,
                    &edge.to,
                    exchange,
                    "outgoing",
...
mod tests {
    ...
    fn summarizes_direct_exchange_interactions() {
        ...
        let summaries =
            exchange_summaries("wallet", &[edge], &metadata);
