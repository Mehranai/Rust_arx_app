    ...
    for address in candidates {
        if let Some(seed) = detect_exchange(&address) {
            push_detection(
                ...
                None,
                seed.cluster_id.unwrap_or_else(|| entity_id(&seed.exchange_name)),
                "seed",
            );
...
fn build_address_row(
    ...
    ExchangeAddressRow {
        ...
        entity_id: entity_id(exchange_name),
...
fn entity_id(exchange_name: &str) -> String {
...
async fn load_address_stats(
    clickhouse: &Client,
    address: &str,
) -> anyhow::Result<AddressStats> {
    let row = clickhouse
        .query(
            r#"
            SELECT
