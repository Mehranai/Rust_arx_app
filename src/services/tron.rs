...
use crate::utils::tron_classification::{
    SimpleTransfer,
    detect_swaps_advanced,
    detect_bridges,
};

//contract classifier
use crate::utils::tron_classifier::types::ClassificationInput;
use crate::utils::tron_classifier::types::ContractType;
use crate::utils::tron_classifier::classifier::classify;
use crate::progress::progress::{
...
// risk engine
use crate::utils::tron_risk_engine::compute_risk_score;
use crate::progress::progress::save_transaction_risk;
...
fn extract_trc20_transfers(
    ...
    for (i, log) in logs.iter().enumerate() {
        ...
        let token = log["address"].as_str().unwrap_or_default();
...
async fn save_wallet_tron(
    ...
    let account = {
        ...
        loader.tron_client.get_account(&addr).await?
    };
...
async fn process_tx(
    ...
    // First Step (Classify Contract)

    // Contract Address Classifier
    ...
    if !contract_address.is_empty() {
        ...
        save_contract_metadata(loader.clickhouse.clone(), row).await?;
    }

    // ✅ AML per transaction
    if !simple_transfers.is_empty() {

        let swaps = detect_swaps_advanced(&simple_transfers);
    ...
    for addr in wallets {
        save_wallet_tron(loader.clone(), addr).await?;
    }
    ...
    while current_block <= latest_block {
        ...
        for tx in txs {
            ...
            tasks.push(tokio::spawn(async move {
                process_tx(loader_clone, tx_clone, block_number).await
            }));
